...
type attachedVolume struct {
	...
	// This field is only used to generate messages when umounting the device.
	spec *volume.Spec
...
type mountedPod struct {
	...
	// in the mount path:
	// /var/lib/kubelet/pods/{podUID}/volumes/{escapeQualifiedPluginName}/{volumeSpecName}/
	// We put this field here to make sure according volume can be umounted correctly,
	// see issue: https://github.com/kubernetes/kubernetes/issues/61248.
	spec *volume.Spec
}
...
func (asw *actualStateOfWorld) MarkRemountRequired(
	...
	for volumeName, volumeObj := range asw.attachedVolumes {
		for mountedPodName, podObj := range volumeObj.mountedPods {
			...
			volumePlugin, err :=
				asw.volumePluginMgr.FindPluginBySpec(podObj.spec)
			if err != nil || volumePlugin == nil {
