...
func (a *flexVolumeAttacher) MountDevice(spec *volume.Spec, devicePath string, _ *v1.Pod) (string, error) {
	...
	if isCmdNotSupportedErr(err) {
		...
		if devicePath != "" {
			return deviceMountPath, (*attacherDefaults)(a).MountDevice(spec, devicePath, deviceMountPath, a.plugin.host.GetMounter(a.plugin.GetPluginName()))
		} else {
			return deviceMountPath, nil
		}
	}
